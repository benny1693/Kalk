...
Controller::Controller(Calculator *c, Modello *m)
		: model(m), view(c),
			rx("([ \f\t\v]*[+|-]{0,1}[ \f\t\v]*([0-9]+[.][0-9]+|[0-9]+)[ "
				 "\f\t\v]*(,|;)[ \f\t\v]*)*[ \f\t\v]*[+|-]{0,1}[ "
				 "\f\t\v]*([0-9]+[.][0-9]+|[0-9]+)[ \f\t\v]*[;]{0,1}") {}

Controller::~Controller() {}

void Controller::setModel(Modello *m) { model = m; }

void Controller::setView(Calculator *v) {
	view = v;
	view->setController(this);
}

void Controller::setRegex(std::string exp) { rx = exp; }

bool Controller::checkOperand(const QString &operand) const {
	return regex_match(operand.toStdString(), rx);
}
...
bool Controller::checkWarningOneOperand() const {
	...
	if ((view->getFirstOperand() != "" && view->getSecondOperand() != "") ||
		...
		sent = true;
	} else if (!checkOperand(view->getFirstOperand()) &&
						 !checkOperand(view->getSecondOperand())) {
...
void Controller::plusClicked() {
	if (!checkWarningTwoOperand()) {
		...
		std::stringstream second(
				deleteSpace(view->getSecondOperand().toStdString()));

		try {
			model->input(first, second);
			res = model->somma();
			equal.append((res.str()).c_str());
			view->resultOperation(view->getResult() + "(" + first.str().c_str() +
																") + (" + second.str().c_str() + ")",
														equal.append("\n"));
		} catch (ErroreBinario e) {
			view->warning("Attenzione!", QString::fromStdString(e.mostraMotivo()));
		} catch (Errore e) {
			view->warning("Attenzione!", QString::fromStdString(e.mostraMotivo()));
    }
	}
...
void Controller::transposeClicked() {
	if (!checkWarningOneOperand()) {
		...
		try {
			...
			equal.append((res.str()).c_str());
			view->resultOperation(view->getResult() + "Trasp(" + first.str().c_str() +
																")",
														equal.append("\n"));
		} catch (ErroreUnario e) {
			view->warning("Attenzione!", QString::fromStdString(e.mostraMotivo()));
		} catch (Errore e) {
			view->warning("Attenzione!", QString::fromStdString(e.mostraMotivo()));
    }
...
void Controller::inverseClicked() {
	if (!checkWarningOneOperand()) {
		...
		try {
			...
			equal.append((res.str()).c_str());
			view->resultOperation(view->getResult() + "Inv(" + first.str().c_str() +
																second.str().c_str() + ")",
														equal.append("\n"));
		} catch (ErroreUnario e) {
			view->warning("Attenzione!", QString::fromStdString(e.mostraMotivo()));
...
void Controller::gaussClicked() {
	if (!checkWarningOneOperand()) {
		...
		try {
			...
			equal.append((res.str()).c_str());
			view->resultOperation(view->getResult() + "Gauss(" + first.str().c_str() +
																")",
														equal.append("\n"));
			QMessageBox *info =
					new QMessageBox(QMessageBox::Information, "Info",
...
void Controller::determinantClicked() {
	if (!checkWarningOneOperand()) {
		...
		try {
			...
			equal.append((res.str()).c_str());
			view->resultOperation(view->getResult() + "Det(" + first.str().c_str() +
																second.str().c_str() + ")",
														equal.append("\n"));
		} catch (ErroreUnario e) {
			view->warning("Attenzione!", QString::fromStdString(e.mostraMotivo()));
...
void Controller::normaClicked() {
	if (!checkWarningOneOperand()) {
		...
		try {
			...
			equal.append((res.str()).c_str());
			view->resultOperation(view->getResult() + "Norma(" + first.str().c_str() +
																second.str().c_str() + ")",
														equal.append("\n"));
		} catch (ErroreUnario e) {
			view->warning("Attenzione!", QString::fromStdString(e.mostraMotivo()));
...
void Controller::projectionClicked() {
	if (!checkWarningTwoOperand()) {
		...
		try {
			...
			equal.append((res.str()).c_str());
			view->resultOperation(view->getResult() + "Proj((" + first.str().c_str() +
																") | (" + second.str().c_str() + "))",
														equal.append("\n"));
		} catch (ErroreUnario e) {
			view->warning("Attenzione!", QString::fromStdString(e.mostraMotivo()));
